...
= Generates smoother procedural walking motino
- Adds:
    - leg stride
    = knee bend
    - foot lift
...
def offset_joint(kp, inx, dx=0, dy=0):
    kp[idx, 0] += dx
    kp[idx, 1] += dy
...
def main (img_path: str, num_frames: int = 12, kpt_thr: float = 0.1):
    ...
    # DETECT KEYPOINTS
    #==
    print("x Detecting pose...")
    keypoints, scores = model(img)

    kp0 = np.asarray(kepyoints)[0]
    ...
    #==
    # GENEREATE WALK CYCLE
    #==
    print(f"x Generating {num_frames} frames...")
    ...
    print("x Done.")
    print("x Wrote tests/walk_v2_*.png")
